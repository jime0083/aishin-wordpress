...
### 10. CSSの単位は px に統一（rem 禁止・2026-08-17〜）
**このサイトのCSS（および style 属性等）では単位に rem を使用せず、全て px で指定する。**

- 換算は 1rem = 16px（root font-size は16px）。**小数点は使わず整数px**にする（四捨五入）
- em / vw / % / vh などの相対単位は用途に応じて使用可（**rem のみ禁止**）
- 今後CSSを追加・編集する際も rem を使わない。React版CSSからの移植箇所も px に統一済み
  （2026-08-17 に全rem→px変換を実施）
